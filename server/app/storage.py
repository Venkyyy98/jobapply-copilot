...
class Storage:
        ...
        with self._conn() as conn:
            cur = conn.execute(
                ...
                INSERT INTO jobs (
                    ...
                    status, outputs, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    payload.get("url", ""),
    ...
    def list_jobs(self, limit: int = 20) -> list[dict[str, Any]]:
        with self._conn() as conn:
            rows = conn.execute(
                "SELECT id, url, title, company, status, created_at, updated_at, fit_score, company_issue_brief, outputs FROM jobs ORDER BY id DESC LIMIT ?",
                (limit,),
            ).fetchall()
    ...
    def list_applied_jobs(self) -> list[dict[str, Any]]:
        with self._conn() as conn:
            rows = conn.execute(
                """
                ...
                """,
                (JobStatus.MANUAL_SUBMISSION_REQUIRED.value,),
            ).fetchall()
        return [dict(row) for row in rows]

    def upsert_feed_item_from_job(self, job_id: int, visibility: str = "public") -> int | None:
    ...
    def upsert_feed_item_from_payload(self, payload: dict[str, Any]) -> int:
        ...
        row_payload = (
            ...
            json.dumps(payload.get("compliance_notes", [])),
            str(payload.get("visibility", "public")).strip() or "public",
            now,
            now,
        )
        with self._conn() as conn:
    ...
    def get_or_create_user(self, email: str, name: str = "", image_url: str = "", provider: str = "google") -> dict[str, Any]:
        ...
        with self._conn() as conn:
            ...
            if row:
                conn.execute(
                    "UPDATE users SET name = ?, image_url = ?, provider = ?, updated_at = ? WHERE email = ?",
                    (name.strip(), image_url.strip(), provider.strip() or "google", now, normalized_email),
                )
            else:
                conn.execute(
                    """
                    INSERT INTO users (email, name, image_url, provider, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
